...
if __name__ == "__main__":
    ...
    #bg whiteboard needs to be saved from getWhiteboard or getWBForeground
    #maybe save background if nothing is in background range
    bg_whiteboard = cv2.imread("Background2.jpg")

    #BUFFERAREA IS AREA CUTOUT FROM OG IMAGE
    bufferIm, bufferArea = getWhiteboard(image)
    fg_mask = getWBForeground(bufferIm, bg_whiteboard)
    display_wb = replaceFG(fg_mask,bg_whiteboard,bufferIm)
    display_final = reattachBG(image, display_wb, bufferArea)

    cv2.imshow("Result", display_final)
